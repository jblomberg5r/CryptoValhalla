...
export const AddTransactionForm: React.FC<AddTransactionFormProps> = ({ onSuccess }) => {
  ...
  const form = useForm<TransactionFormValues>({
    ...
    defaultValues: {
      ...
      quantity: undefined,
      pricePerCoin: undefined,
      currencyAtTransaction: "USD",
      exchange: "",
      fees: undefined,
      notes: "",
    },
  ...
  const onSubmit = async (data: TransactionFormValues) => {
    ...
    try {
      ...
      form.reset({
        ...
        quantity: undefined,
        pricePerCoin: undefined,
        currencyAtTransaction: "USD", // Or from global user preference store
        exchange: "",
        fees: undefined,
        notes: "",
      });
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <Popover open={coinSearchOpen} onOpenChange={setCoinSearchOpen}>
                ...
                <PopoverContent className="w-[--radix-popover-trigger-width] max-h-[--radix-popover-content-available-height] p-0">
                  <Command filter={(value, search) => {
                    // Default filter is fine, but if more complex matching is needed, implement here
                    // console.log(`Filtering: value='${value}', search='${search}'`);
                    ...
                    <CommandList>
                      ...
                      <CommandGroup>
                        {coinList.map((coin) => (
                          <CommandItem
                            key={coin.id}
                            value={`${coin.name} (${coin.symbol}) ${coin.id}`} // Value used for filtering
                            onSelect={() => {
        ...
        <Button type="submit" disabled={isLoading || isFetchingCoins} className="w-full">
